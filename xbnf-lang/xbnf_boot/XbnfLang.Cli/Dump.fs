...
let dumpTerm term acc =
  let rec go (term, (annotations, location)) (superBp: Bp) acc =
    ...
    | [] ->
      ...
      match term with
      | TokenTerm name -> acc |> cons name

      | SymbolTerm name -> acc |> cons name
...
let dumpStmt stmt acc =
  ...
  | RuleStmtTerm (name, body, comments, _) ->
    let acc =
      comments
      |> List.fold (fun acc comment -> acc |> cons "# " |> cons comment |> cons eol) acc

    let altHead =
      String.replicate (name.Length + 1) " " + "| "
    ...
    let bodies =
      let rec go body =
        match body with
        | OrTerm (first, second), _ -> List.append (go first) (go second)
        | _ -> [ body ]
    ...
    |> List.fold
         (fun (head, acc) body ->
           let acc =
             acc |> cons head |> dumpTerm body |> cons eol
