...
type FSharpTerm =
  | IdentFT of string
  | StringFT of string
  | ListFT of FSharpTerm list
  | TupleFT of FSharpTerm list
  | BinFT of FSharpBin * FSharpTerm * FSharpTerm

type FSharpPat =
  | TermFP of FSharpTerm
  | OrFP of FSharpPat * FSharpPat

type FSharpExpr = FSharpTerm

type FSharpStmt =
  | TermFS of FSharpTerm
  | MatchFS of cond: FSharpTerm * arms: (FSharpPat * FSharpExpr option * FSharpStmt list) list
  | LetValFS of FSharpPat * FSharpStmt list
  | LetFunFS of string * string list * body: FSharpStmt list

type FSharpModule = FSharpModule of modulePath: string list * openPaths: string list list * FSharpStmt list

type FSharpLiteOption = FSharpLiteOption of modulePath: string list * openPaths: string list list

let appFT first second = BinFT(AppFB, first, second)

let appManyFT first args = args |> List.fold appFT first

let consFT first second = BinFT(ConsFB, first, second)

let andFT first second = BinFT(AndFB, first, second)

let orFT first second = BinFT(OrFB, first, second)

let consFP first second = TermFP(consFT first second)

let orFP first second = OrFP(first, second)
...
let snakeToCamelCase (name: string) =
  if name = "" || name.StartsWith("_") then
    name
  else
...
let tokenToTerm (name: string) args =
  ...
  if name.[0] = '"' || name.[0] = '\'' then
    ...
    k
      (IdentFT "PunToken")
      ((name.Substring(1, name.Length - 2) |> StringFT)
       :: args)
  else
    k (snakeToPascalCase name + "Token" |> IdentFT) args

let tokenDataTerm (name: string) args data = TupleFT [ tokenToTerm name args; data ]
...
let fsharpLiteGenIsFollowedByFuns rules =
  ...
  let halt (tokens, symbolOpt, _) = tokens, symbolOpt, true

  let rec goLook (node: NodeData) =
    match node with
    | TokenNode name, _ -> [ [ name ], None, false ]

    | SymbolNode name, _ -> [ [], Some name, true ]

    | EmptyNode, _ -> [ [], None, false ]
    ...
    | ConcatNode (first, second), _ ->
      ...
      |> List.collect (fun first ->
        second
        |> goLook
        |> List.map (fun second -> append first second))

    | OrNode (first, second), _ -> List.append (goLook first) (goLook second)
  ...
  let gen rule =
    ...
    | Rule (symbol, body, _, _) ->
      let arms =
        ...
        |> List.map (fun ((tokens, symbolOpt, _), _) ->
          ...
          let guard =
            ...
            | Some symbol ->
              let funTerm =
                IdentFT("isFollowedBy" + snakeToPascalCase symbol)

              appFT funTerm tokensTerm |> Some

            | None -> None

          let body = IdentFT "true"
      ...
      let funName =
        "isFollowedBy" + snakeToPascalCase symbol
  ...
  let rec go k (node: NodeData) =
      ...
      k (
        MatchFS(
          ...
          [
            ...
            // | _ ->
            //   error
            TermFP(IdentFT "_"), None, [ TermFS(raiseTerm name) ] ]
        )
      )
    ...
    | SymbolNode name, _ ->
      ...
      k (
        TermFS(
          appManyFT
            ("parse" + snakeToPascalCase name |> IdentFT)
            [ IdentFT "emit"
              IdentFT "tokens"
              IdentFT "state" ]
        )
    ...
    | ConcatNode _, _ ->
      let rec flatten node =
        match node with
        | ConcatNode (first, second), _ -> List.append (flatten first) (flatten second)
        | _ -> [ node ]
    ...
    | OrNode ((Many1Node item, _), (EmptyNode _, _)), _ ->
      ...
      let goTerm =
        appManyFT (IdentFT "go") [ IdentFT "tokens"; IdentFT "state" ]
          ...
          [ MatchFS(
              ...
              [ match item with
                | ConcatNode ((TokenNode token, _), item), _ ->
                  ...
                  // | _ -> tokens, state
                  TermFP __, None, [ TermFS pairTerm ]
                ...
                | _ ->
                  ...
                  let pat =
                    ...
                    |> Set.toSeq
                    |> Seq.map (fun token ->
                      consFT (TupleFT [ tokenToTerm token [ __ ]; __ ]) __
                      |> TermFP)
                    |> Seq.reduce orFP
    ...
    | OrNode _, _ ->
      let rec flatten node =
        match node with
        | OrNode (first, second), _ -> List.append (flatten first) (flatten second)
      ...
      k (
        MatchFS(
          ...
          [ for node in flatten node do
              match node with
              | EmptyNode _, _ ->
                TermFP(IdentFT "_"), None, [ TermFS pairTerm ]
  ...
  let rec go term eol (superBp: FSharpBindingPower) acc =
    ...
    | TupleFT items ->
      ...
      let _, acc =
        ...
        |> List.fold
             (fun (sep, acc) item ->
               let acc = acc |> cons sep |> go item eol MinFbp
               ", ", acc)
             ("", acc)
    ...
    | ListFT [ item ] -> acc |> cons "[" |> go item eol MinFbp |> cons "]"
    ...
    | ListFT items ->
      ...
      let acc =
        items
        |> List.fold (fun acc item -> acc |> cons deepEol |> go item deepEol MinFbp) acc
...
let fsharpLiteDumpPat pat eol acc =
  match pat with
  | TermFP term -> acc |> fsharpLiteDumpTerm term eol
...
let fsharpLiteDumpStmt stmt eol acc =
  ...
  match stmt with
  | TermFS term -> acc |> fsharpLiteDumpTerm term eol
  ...
  | LetFunFS (name, args, body) ->
    let acc = acc |> cons "let " |> cons name

    let acc =
      args
      |> List.fold (fun acc name -> acc |> cons " " |> cons name) acc
...
let fsharpLiteDumpSemi stmts eol acc =
  ...
  |> List.fold
       (fun (sep, acc) stmt ->
         let acc =
           acc |> cons sep |> fsharpLiteDumpStmt stmt eol
