...
let ruleToName rule =
  match rule with
  | Rule (name, _, _, _) -> name

let ruleAppend first second =
  match first, second with
  | Rule (name, body, comments, location), Rule (secondName, secondBody, secondComments, secondLocation) ->
    assert (name = secondName)

    let body =
      OrNode(body, secondBody), (noAnnotation, secondLocation)
...
let ruleConcat rules =
  ...
  | rule :: rules -> rules |> List.fold ruleAppend rule

let rec lowerTerm (term: TermData) =
  match term with
  | (TokenTerm "\"\"", location)
  | (TokenTerm "''", location) -> EmptyNode, location

  | TokenTerm name, location -> TokenNode name, location

  | SymbolTerm name, location -> SymbolNode name, location
  ...
  | SepTerm (item, sep), location ->
    ...
    let sep =
      TokenNode(sprintf "\"%s\"" sep), location
...
let lowerStmt stmt =
  match stmt with
  | RuleStmtTerm (name, body, comments, location) -> Rule(name, lowerTerm body, comments, location)
