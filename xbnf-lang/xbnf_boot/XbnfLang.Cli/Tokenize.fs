...
let charIsCommentFirst (c: char) = c = '#' || c = '/'
...
let charIsIdentFirst (c: char) = c = '_' || charIsAlpha c
...
let charIsOther (c: char) = charIsAnyFirst c |> not
...
let lookOther (text: string) (i: int) = text.[i] |> charIsOther
